...
async function addProperty(formData: FormData): Promise<void> {
  ...
  const images = formData
    ...
    .filter(
      (entry) =>
        (entry as ImageFile).name !== '' && (entry as ImageFile).size !== 0,
    );
  ...
  const propertyDataFromForm: Omit<PropertyApiPost, 'owner' | 'images'> = {
    ...
    rates: {
      nightly: formData.get('rates.nightly')
        ? Number.parseFloat(formData.get('rates.nightly') as string)
        : undefined,
      weekly: formData.get('rates.weekly')
        ? Number.parseFloat(formData.get('rates.weekly') as string)
        : undefined,
      monthly: formData.get('rates.monthly')
        ? Number.parseFloat(formData.get('rates.monthly') as string)
        : undefined,
    },
    seller_info: {
